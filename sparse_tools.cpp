...
static inline double f_by_ij (double i, double j, int n, int m, func2d &f, grid_data grid)
{
...
static inline int get_index_by_i_j (int i, int j, int , int m)
{
  return i * (m + 1) + j;
}

static inline void vector_product (double v[], double u[], double r[])
{
  r[0] = u[1] * v[2] - u[2] * v[1];
  r[1] = u[2] * v[0] - u[0] * v[2];
  r[2] = u[0] * v[1] - u[1] * v[0];
...
void create_rhs (int n, int m, int matrix_size, func2d &func, double *rhs,
  ...
  for (int l = l1; l <= l2; l++)
    {
      int i, j;
      get_i_j_by_index (l, i, j, n, m);
      get_func_values_neighbourhood (i, j, n, m, func, close_vals, edge_vals, vals, grid);
  ...
  if (delt_y > delt_x) // upper triangle
    {
      ...
      i++; index = get_index_by_i_j (i, j, n, m);
  ...
  else
    {
      ...
      i++; j++; index = get_index_by_i_j (i, j, n, m);
...
void MSR_mtx_mult_vector (double *A, int *I, int n, double *x, double *y, int p, int k)
{
  int i1, i2;
...
int MSR_solve (double *a, int *I, int n, double *x, double *b, double *r, double *u, double *v,
...
double MSR_residual (int n, int m, double *vals,
                     func2d &f, double &max, grid_data grid, int p, int k)
{
